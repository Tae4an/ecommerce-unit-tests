...
public class Main {
        ...
        while (true) {
            ...
            try {
                switch (choice) {
                    ...
                    case 2 -> handleCartMenu();
                    case 3 -> handleWishlistMenu();
                    case 4 -> Utils.writeReview(loggedInCustomer);
                    case 5 -> {
    ...
    private static void handleAdminMenu() {
        while (true) {
            ...
            try {
                switch (choice) {
                    case 1 -> handleAdminCustomerMenu();
                    case 2 -> handleAdminBoardMenu();
                    case 3 -> handleAdminDeliveryMenu();
                    case 4 -> handleAdminMileageMenu();
                    case 5 -> {
                        System.out.println("로그아웃 되었습니다.");
                        loggedInCustomer = null;
                        return;
    ...
    private static void handleAdminCustomerMenu() throws Exception {
        while (true) {
            System.out.println("\n=== 회원 관리 ===");
    ...
    private static void handleAdminBoardMenu() throws Exception {
        while (true) {
            System.out.println("\n=== 게시글 관리 ===");
            ...
            int choice = getChoice(4);
            switch (choice) {
                case 1 -> Utils.listBoards();
                case 2 -> Utils.getBoard();
                case 3 -> Utils.deleteBoard();
                case 4 -> {
                    return;
                }
            }
    ...
    private static void handleAdminDeliveryMenu() throws Exception {
        while (true) {
            System.out.println("\n=== 배송 관리 ===");
            System.out.println("1. 배송 목록 조회");
            System.out.println("2. 배송 정보 조회");
            System.out.println("3. 배송 정보 수정");
            System.out.println("4. 주문별 배송 조회");
            System.out.println("5. 이전 메뉴로");
            System.out.print("선택: ");

            int choice = getChoice(5);
            switch (choice) {
                case 1 -> Utils.listDeliveries();
                case 2 -> Utils.getDelivery();
                case 3 -> Utils.updateDelivery();
                case 4 -> Utils.getDeliveriesByOrderId();
                case 5 -> {
                    return;
                }
            }
    ...
    private static void handleAdminMileageMenu() throws Exception {
        while (true) {
            System.out.println("\n=== 마일리지 관리 ===");
            System.out.println("1. 마일리지 목록 조회");
            System.out.println("2. 마일리지 조회");
            System.out.println("3. 마일리지 수정");
            System.out.println("4. 마일리지 삭제");
            System.out.println("5. 마일리지 추가");
            System.out.println("6. 이전 메뉴로");
            System.out.print("선택: ");

            int choice = getChoice(6);
            switch (choice) {
                case 1 -> Utils.listMileages();
                case 2 -> Utils.getMileage();
                case 3 -> Utils.updateMileage();
                case 4 -> Utils.removeMileage();
                case 5 -> Utils.addMileagePoints();
                case 6 -> {
                    return;
                }
            }
